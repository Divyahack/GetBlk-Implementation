...
DLL HQ[4],FL;              //  HQ[] & FL of type Doubly Linked List from dll.h
node *temp=new node;
mutex mtx;                  //mutex uses the lock-based technique 
int r_hqnum;                //to store hashqueue number for scenario 5
int b5;                      //to store block number for scenario 5 
  ...
  lb:
      switch(r_w)
      {
       ...
       case 1:                       //To handle Write Operation
          ...
          else
          {
	           ...
	           {                    
                 ...
                 Insert_at_start=1;
		         PrevTemp=temp->data;
	             goto lb;        // to continue again & get another from free list  
	           } 
	          ...
	          { 
		         ...
		         if(HQ[HQnum].isPresent(blockNum)) 
				    goto l12;
		       	HQ[HQnum].insert_at_end(blockNum,1,2);           // busy & DW
		        mtx.unlock();
 			    l12:
		        cout<<"\nBuffer is locked for : "<<std::this_thread::get_id()<<endl;		  
		        handle_write(HQnum,blockNum);
	   ...
	   case 2:                               // To handle Read Operation
	    ...
	    {
		  FL.insert_at_start(PrevTemp,0,1);
		  HQ[PrevTemp%4].change_status2(PrevTemp,1);        //mark valid
	    }	
	    break;
 	}
